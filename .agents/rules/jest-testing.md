...
- **目录约定**：优先将测试文件放在与被测代码同级的 `__test__` 目录中，便于统一组织与检索；如项目现有工具链已允许，也可使用符合约定的测试文件命名放置在其他受支持的位置。
- 测试文件后缀命名应使用 `.test.ts`、`.test.tsx` 或 `.test.js`，并以项目当前 `tsconfig` / ESLint / Jest 配置实际匹配的 glob 规则为准。
- 若未放在 `__test__` 目录中，仍需确保文件路径与命名满足仓库测试配置要求，避免出现文档约定与工具链实际行为不一致的情况。
