...
describe('Context build: CLAUDE.md + system prompt integration', () => {
  ...
  test('buildEffectiveSystemPrompt with appendSystemPrompt includes both', () => {
    ...
    expect(joined.indexOf('Main prompt')).toBeLessThan(joined.indexOf('Appended'));
  });
});
...
describe('Context build: CLAUDE.md file system integration', () => {
  ...
  test('isMemoryFilePath correctly identifies CLAUDE.md paths', () => {
    expect(isMemoryFilePath(join('/', 'project', 'CLAUDE.md'))).toBe(true);
    expect(isMemoryFilePath(join('/', 'project', 'CLAUDE.local.md'))).toBe(true);
    expect(isMemoryFilePath(join('/', 'project', '.claude', 'rules', 'file.md'))).toBe(true);
    expect(isMemoryFilePath(join('/', 'project', 'README.md'))).toBe(false);
    expect(isMemoryFilePath(join('/', 'project', 'src', 'index.ts'))).toBe(false);
  });
});
...
describe('Context build: large memory file filtering', () => {
  ...
  test('getLargeMemoryFiles returns empty when all files are small', () => {
    const files = [
      { type: 'file' as const, path: '/a/CLAUDE.md', content: 'small' },
      { type: 'file' as const, path: '/b/CLAUDE.md', content: 'also small' },
    ] as any; // Using any or exactly what MemoryFileInfo expects
