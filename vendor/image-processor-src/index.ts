...
export function getNativeModule(): NativeModule | null {
  ...
  try {
    ...
    cachedModule = process.env.IMAGE_PROCESSOR_NODE_PATH
      ? require(process.env.IMAGE_PROCESSOR_NODE_PATH)
      : require(`../image-processor/${process.arch}-${process.platform}/image-processor.node`)
  } catch {
    cachedModule = null
  }
  return cachedModule
